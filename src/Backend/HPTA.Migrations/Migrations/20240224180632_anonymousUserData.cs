...
{
    /// <inheritdoc />
    public partial class anonymousUserData : Migration
    {
        /// <inheritdoc />
...
WITH DistinctUsersCTE AS (
		...
		FROM
			Teams
			INNER JOIN UserTeams ON Teams.Id = UserTeams.TeamId AND UserTeams.IsCoreMember = 1
...
END";
            ...
            var dropSp = @" DROP PROCEDURE [dbo].[Usp_GetAnonymousUserData];
GO";

            migrationBuilder.Sql(categoryWise);
            migrationBuilder.Sql(teamWise);
            migrationBuilder.Sql(userChart);
            migrationBuilder.Sql(categoryChart);
