...
{
    public class HashingService(string secretKey) : IHashingService
    {
        public string GenerateHash(string password, KeyDerivationPrf prf = KeyDerivationPrf.HMACSHA256, int iterationCount = 10000, int saltSize = 16)
        {
            using (var rng = RandomNumberGenerator.Create())
            {
                ...
                var pbkdf2Hash = KeyDerivation.Pbkdf2(secretKey + password, salt, prf, iterationCount, 32);
        ...
        public bool VerifyHash(string password, string passwordHash)
        {
            ...
            var hashFromInputPassword = KeyDerivation.Pbkdf2(secretKey + password, salt, prf, iterationCount, 32);
